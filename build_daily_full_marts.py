...
FULL_MARTS_S3_BASE = "s3://kestra-zoomcamp-ny-taxi-mtl9999999999988888/powerbi_daily_full_marts/"
PRODUCT_FULL_PREFIX = "product/"
DIVISION_FULL_PREFIX = "division/"

PRODUCT_FULL_TABLE_NAME = "mart_product_cfo_daily_full"
DIVISION_FULL_TABLE_NAME = "mart_division_cfo_daily_full"
...
def ensure_full_daily_tables_exist() -> None:
    product_location = f"{FULL_MARTS_S3_BASE.rstrip('/')}/{PRODUCT_FULL_PREFIX}"
    division_location = f"{FULL_MARTS_S3_BASE.rstrip('/')}/{DIVISION_FULL_PREFIX}"

    drop_product_sql = f"DROP TABLE IF EXISTS {ATHENA_DATABASE}.{PRODUCT_FULL_TABLE_NAME}"
    drop_division_sql = f"DROP TABLE IF EXISTS {ATHENA_DATABASE}.{DIVISION_FULL_TABLE_NAME}"

    product_sql = f"""
    CREATE EXTERNAL TABLE {ATHENA_DATABASE}.{PRODUCT_FULL_TABLE_NAME} (
        date string,
        day string,
        week string,
        month string,
    ...
    division_sql = f"""
    CREATE EXTERNAL TABLE {ATHENA_DATABASE}.{DIVISION_FULL_TABLE_NAME} (
        date string,
        day string,
        week string,
        month string,
...
def build_and_publish_daily_full_marts() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Force daily granularity while keeping the full dataset
    scope = {"mode": "days"}
    ...
    product_s3_uri = f"{FULL_MARTS_S3_BASE.rstrip('/')}/{PRODUCT_FULL_PREFIX}"
    division_s3_uri = f"{FULL_MARTS_S3_BASE.rstrip('/')}/{DIVISION_FULL_PREFIX}"

    delete_s3_prefix(product_s3_uri)
    delete_s3_prefix(division_s3_uri)

    upload_df_as_csv_to_s3(product_out, product_s3_uri, "product_daily_full.csv")
    upload_df_as_csv_to_s3(division_out, division_s3_uri, "division_daily_full.csv")

    ensure_full_daily_tables_exist()
...
if __name__ == "__main__":
    ...
    division_df, product_df = build_and_publish_daily_full_marts()

    print("\n==============================")
    print("FULL DIVISION DAILY MART")
    print("==============================")
    print(
        division_df[["date", "day", "week", "month"]]
        .drop_duplicates()
        .sort_values("date")
    ...
    print("\n==============================")
    print("FULL PRODUCT DAILY MART")
    print("==============================")
    print(
        product_df[["date", "day", "week", "month"]]
        .drop_duplicates()
        .sort_values("date")
        .to_string(index=False)
    )
    ...
    print("\nRows written:")
    print("division =", len(division_df))
